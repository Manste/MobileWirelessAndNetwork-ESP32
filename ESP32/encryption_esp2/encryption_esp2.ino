...
void base64Encode(const unsigned char* data, size_t dataLen, unsigned char** encodedData) {
    size_t written = 0, outsize = 0;
    mbedtls_base64_encode(0, 0, &written, data, dataLen);
    *encodedData = (unsigned char*)malloc(sizeof(char) * (written + 1));
    mbedtls_base64_encode(*encodedData, written, &outsize, data, dataLen);
    (*encodedData)[written] = '\0';
}
...
void preparePayload(String data, unsigned char* key, const char* hashKey, Adafruit_MQTT_Publish &publisher) {
  ...
  if(error == 0){ 
    ...
    mbedtls_ctr_drbg_random(&ctr_drbg, iv1, BLOCK_SIZE);
    memcpy(iv2, iv1, BLOCK_SIZE);
    ...
    mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_ENCRYPT, strlen((const char*) paddedData), iv1, (const unsigned char*) paddedData, cipherText);
    mbedtls_aes_free(&aes);
    
    // iv, hash and cipherText to base64
    unsigned char* ivB64 = nullptr;
    base64Encode(iv2, BLOCK_SIZE, &ivB64);
    
    unsigned char* cipherTextB64 = nullptr;
    base64Encode(cipherText, strlen((const char*) paddedData), &cipherTextB64);
    ...
    unsigned char* hashB64 = nullptr;
    base64Encode(hash, 32, &hashB64);
...
void base64Decode(const char* dataB64, unsigned char** data) {
    size_t written = 0, outsize = 0;
    mbedtls_base64_decode(NULL, 0, &written, (const unsigned char*)dataB64, strlen(dataB64));
    *data = (unsigned char*)malloc(sizeof(char)*(written+1));
    mbedtls_base64_decode(*data, written, &outsize, (const unsigned char*)dataB64, strlen(dataB64));
}
...
void setup() {
  ...
  while (!esp1Key || !esp2Key || !serverKey) {    
      base64Decode(ESP1_ENCRYPT, &esp1Key);
      base64Decode(ESP2_ENCRYPT, &esp2Key);
      base64Decode(SERVER_ENCRYPT, &serverKey);
  }
...
void loop() {
  ...
  mqtt.processPackets(3000); // Process incoming messages
